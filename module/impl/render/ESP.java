...
import genius.event.impl.EventNametagRender;
...
public class ESP extends Module {
    ...
    public ESP(ModuleData data) {
        ...
        settings.put(ARMOR, new Setting<>(ARMOR, false, "Shows the armor bars on the left."));
    }
    ...
    @Handler
    public void onNametagRender(EventNametagRender event) {
            ...
            if ((renderPositions[3] > 0.0D) || (renderPositions[3] <= 1.0D)) {
                ...
                if ((((Boolean)settings.get(INVISIBLES).getValue()) || !ent.isInvisible()) && ent instanceof EntityPlayer && !(ent instanceof EntityPlayerSP)) {
                    ...
                    try {
                        ...
                        RenderingUtil.rectangleBordered(x, y, endx, endy, 1.65, Colors.getColor(0, 0, 0, 0), color);
                        RenderingUtil.rectangleBordered(x - 0.5, y - 0.5, endx + 0.5, endy + 0.5, 0.3, Colors.getColor(0, 0), Colors.getColor(0));
                        RenderingUtil.rectangleBordered(x + 2.5, y + 2.5, endx - 2.5, endy - 2.5, 0.3, Colors.getColor(0, 0), Colors.getColor(0));
                        RenderingUtil.rectangleBordered(x - 5, y - 1, x - 1, endy, 0.4, Colors.getColor(0, 100), Colors.getColor(0, 255));
                        if (((Boolean)settings.get(NAME).getValue())) {
                            GlStateManager.pushMatrix();
                            String renderName = FriendManager.isFriend(ent.getName()) ? FriendManager.getAlias(ent.getName()) : ent.getName();
                            TTFFontRenderer font = Client.fontManager.v16b;
                            float meme2 = ((endx - x) / 2 - (font.getStringWidth(renderName) / 2f));
                            font.drawStringWithShadow(renderName, (x + meme2), (y - font.getStringHeight(renderName) - 5), FriendManager.isFriend(ent.getName()) ? Colors.getColor(192,80,64) : -1);
                            GlStateManager.popMatrix();
                        }
                        if (((EntityPlayer) ent).getCurrentEquippedItem() != null && ((Boolean) settings.get(ITEMS).getValue())) {
                            ...
                            TTFFontRenderer font = Client.fontManager.v12;
                        ...
                        if ((int) MathUtils.getIncremental(health * 5, 1) != 100 && ((Boolean) settings.get(HEALTH).getValue())) {
                            ...
                            TTFFontRenderer font = Client.fontManager.v12;
                            font.drawStringWithShadow(nigger, (x - 5 - font.getStringWidth(nigger) * 2) / 2, ((float) ((int) healthLocation) + font.getStringHeight(nigger) / 2) / 2, -1);
                            GlStateManager.popMatrix();
                        }
                    } catch (Exception e) {
                    }
    ...
    private void scale(Entity ent) {
        ...
        this.gradualFOVModifier += (target - this.gradualFOVModifier) / (Minecraft.getMinecraft().getDebugFPS() * 0.7D);
